...
\centering
\vspace*{0.6cm}
{\large\heb{הפקולטה למדעי המחשב והמידע ע"ש שטיין}\par}
\vspace{3pt}
{\large\heb{אוניברסיטת בן-גוריון בנגב}\par}
\vspace{3pt}
{\large\heb{התוכנית להנדסת נתונים}\par}
\vspace{1.6cm}
...
and the pattern of its one-hop transactions. The model is trained on a combined corpus of two
public labelled datasets, \textbf{REAL-CATS} (modern criminal wallets) and
\textbf{Elliptic++} (theft/Ponzi era). Restricting to the wallets for which a one-hop
ego-graph could be built and cached yields a benign-majority working corpus of 65{,}849
training wallets and 11{,}880 held-out test wallets. On the held-out test set the GNN reaches an \textbf{F\textsubscript{1}
...
\vspace{1.4em}
{\raggedleft\LARGE\bfseries\heb{תקציר}\par}
...
\vspace{4pt}
\hebpar{הגדרנו את הבעיה כסיווג בינארי של צמתים מעל גרפי-אֶגו של ארנקים בודדים, ואימנו רשת קשב גרפית מסוג GATv2 בת שלוש שכבות הקוראת גם את התנהגות הארנק עצמו וגם את דפוס העסקאות של שכניו הישירים. המודל אומן על מיזוג של שני מאגרי נתונים ציבוריים מתויגים: REAL-CATS (ארנקים פליליים עדכניים) ו-Elliptic++. לאחר צמצום לארנקים שעבורם ניתן היה לבנות ולשמור גרף-אֶגו, מתקבל קורפוס עבודה שבו רוב הארנקים תמימים, בסך 65,849 ארנקים לאימון ו-11,880 ארנקים למבחן. על קבוצת המבחן השיג המודל ציון F1 של 0.869, דיוק 0.894, היזכרות 0.884 ו-ROC-AUC של 0.959, עם הסתברויות מכוילות היטב לאחר כיול טמפרטורה. המודל עקף גם מסווג XGBoost שאומן על אותם שנים-עשר מאפיינים טבלאיים (F1 של 0.755) וגם בסיס GCN פשוט (F1 של 0.582). הפער של כ-11 נקודות ב-F1 על פני XGBoost הוא הממצא המרכזי: המבנה המקומי של העסקאות סביב הארנק נושא מידע שאין במאפייני הארנק עצמו. המודל המאומן מוגש דרך שרת FastAPI ואפליקציית React שנפרסו בכתובת cryptotrace.cs.bgu.ac.il.}
...
REAL-CATS and Elliptic++ into one combined corpus, and compare our model against two
...
calibrated risk verdict. The same model is trained offline on the combined corpus of REAL-CATS
and Elliptic++.}
\label{fig:workflow}
\end{figure}

\section{Main Findings}
On a held-out test set of 11{,}880 wallets, the GNN reaches F\textsubscript{1}~$=0.869$ and
...
\begin{description}[leftmargin=1.3em,style=nextline,itemsep=3pt]
  ...
  \item[G2 --- Unified corpus.] Merge REAL-CATS and Elliptic++ onto a common
    feature schema, remove overlap and label conflicts, sample benign controls per source,
    and produce a source-stratified train/test split.
...
adopt imbalance-aware practice by sampling benign controls when merging the sources and encoding a
...
duplicates, samples benign controls within each source, and takes a source-stratified 80/20
split keyed on the joint (label, source) with a fixed seed. Because every model operates on
ego-graphs, the working corpus is then restricted to the wallets for which a one-hop ego-graph
could be fetched and cached; ego-graph construction succeeded more often for benign wallets, so
the corpus actually used for training and evaluation (Table~\ref{tab:split}) retains a benign
majority. Both sources appear in near-identical proportions across the train and test splits
($\sim$82\% REAL-CATS), while the criminal share differs between them because caching dropped the
two classes at different rates.

\begin{table}[H]
\centering\small
\caption{Source-stratified train/test split, reported on the working corpus of wallets with a
cached one-hop ego-graph (the rows actually used by all three models).}
\label{tab:split}
\begin{tabular}{lrrrrr}
\toprule
\textbf{Split} & \textbf{Rows} & \textbf{Criminal} & \textbf{Benign} & \textbf{REAL-CATS} & \textbf{Elliptic++} \\
\midrule
Training set & 65{,}849 & 19{,}206 & 46{,}643 & 54{,}320 & 11{,}529 \\
Test set     & 11{,}880 &  4{,}740 &  7{,}140 &  9{,}778 &  2{,}102 \\
\bottomrule
\end{tabular}
\end{table}
...
form high-degree hubs (Figure~\ref{fig:eda_degree}) --- illicit actors rarely form tightly
connected clusters and instead
...
Figure~\ref{fig:eda_comp} shows the composition of the working corpus: benign-majority and
dominated by REAL-CATS, with both sources kept in near-identical proportion across the train and
test splits. Figure~\ref{fig:eda_degree} makes the structural difference in Elliptic++ concrete:
illicit transaction nodes concentrate at low in- and out-degree, whereas licit nodes reach far
into the high-degree tail. The out-degree gap is the sharpest --- illicit nodes almost never send
to more than two counter-parties, while licit nodes fan out to hundreds --- which is exactly the
neighbourhood structure the graph model is built to read.

\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{figures/fig_eda_composition.pdf}
\caption{Composition of the working corpus (wallets with a cached one-hop ego-graph). Left:
class counts per split; right: source counts per split. The corpus is benign-majority and
$\sim$82\% REAL-CATS in both splits.}
\label{fig:eda_comp}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig_eda_degree.pdf}
\caption{Elliptic++ transaction-graph degree distribution by class (4{,}545 illicit vs.\ 42{,}019
licit nodes), as a percentage of each class's nodes on a log axis. Illicit nodes concentrate at
low degree; licit nodes extend into the high-degree tail. Illicit out-degree tops out near two
(max $3$), whereas licit out-degree reaches $472$ --- the ``high-degree hubs'' that node-level
laundering blends into.}
\label{fig:eda_degree}
\end{figure}
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item \textbf{Controls \& split.} Sample benign controls within each source and take the
    source-stratified 80/20 split of Table~\ref{tab:split} (reported on the ego-graph--cached
...
\texttt{mempool.space} API (up to 50 transactions per wallet) and cached per address; the same
50-transaction cap is used when building the training ego-graphs and at live inference, so the
graph structure the model is trained on matches the structure it sees in production.
...
temperature parameter is fit by LBFGS on the held-out validation set (the 15\% split also used
for early stopping) and applied at inference
...
stopping and best-epoch selection on validation F\textsubscript{1}. The temperature parameter for
post-hoc calibration is fit on this validation set as well (a dedicated calibration split is
left to future work). All three models are trained and
...
Basic GCN   & $3\times$ GCNConv, hidden 64, mean-pool readout, dropout 0.3 \\
...
a point of XGBoost while recovering \emph{far} more criminals (recall 0.884 vs.\ 0.673).
...
give the GNN AUC~$=0.959$, well above XGBoost ($0.909$) and the plain GCN ($0.777$); on the
imbalanced test set the GNN's PR-AUC is $0.941$.
...
Expected Calibration Error, so the risk score the web application displays can be read close to
face value rather than only as a ranking. Because the temperature is fit on the validation set and
the reliability diagram is measured on the held-out test set, this is a fair (not optimistic)
estimate of calibration.
...
its public history, whether a wallet has been used for criminal activity. Trained on a combined
corpus of REAL-CATS and Elliptic++, a three-layer GATv2 network reaches F\textsubscript{1}
...
\section{Future Work}
Natural extensions are: (i) raise the ego-graph transaction cap beyond the current 50 transactions
per wallet --- applied identically at training and inference --- so deeper neighbourhoods are
captured, and rebuild the corpus at that larger depth; (ii) train on the larger graph corpus once
the full download completes; (iii) fit the temperature on a dedicated calibration split instead of
reusing the early-stopping validation set; (iv) build a live adversarial test set of recent wallets in neither source to stress-test
...
repeated API calls; transaction fetching is capped at 50 transactions per wallet and issued
sequentially to respect \texttt{mempool.space} rate limits (the same cap used to build the
training ego-graphs); the UI meets basic accessibility and
...
\end{quote}
or step by step: \texttt{--prepare} (build the dataset) $\rightarrow$ \texttt{--graphs}
